@echo off
@echo starting grid worker
set mainClass=com.traclabs.biosim.ga.grid.JGAPWorker
...
java -Xmx512m -classpath %buildDir%;%resourceDir%;%gaClasspath%;%logCLasspath%;%jacorbClasspath%;%biosimClasspath%;%plotClasspath% -D%jacoOrbClass% -D%jacoSingletonOrbClass% -D%jacoNameIOR% "%mainClass%" %*
pause